...
The **`useGeolocation`** hook gives you programmatic control over geolocation requests without needing a button component. It returns 4 values:

- **`location`** An object containing:  
  - `.latitude`: the user's current latitude, or `null` if not fetched.  
  - `.longitude`: the user's current longitude, or `null` if not fetched.  
  - `.error`: any error message if the geolocation request failed.  

- **`isLoading`** `boolean` indicating whether the geolocation request is in progress.  

- **`fetchLocation()`** Function to trigger a request to get the user's current location.  

- **`resetLocation()`** Function to reset the location state (`latitude` and `longitude` → `null`, `error` → `null`) and stop loading.
